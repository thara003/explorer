...
const LandingPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const result = await client.get('/api/_/global_overview').catch(axios.defaults.errorHandler)

      setAsnCount(toCompactNumberUnit(result.data.network_count))
      setMeasurementCount(toCompactNumberUnit(result.data.network_count))
      setCountryCount(result.data.country_count)
    }
